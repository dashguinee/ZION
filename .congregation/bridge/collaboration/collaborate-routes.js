...
function checkStopConditions(session, requested_stop) {
  ...
  if (session.turns.length >= 3) {
    ...
    const progressChanged = recentTurns.some((turn, i) => {
      if (i === 0) return false;
      const prevProgress = session.turns[session.turns.length - 3 + i - 1].state_analysis?.gap_to_goal?.current_progress || 0;
      const currentProgress = turn.state_analysis?.gap_to_goal?.current_progress || 0;
